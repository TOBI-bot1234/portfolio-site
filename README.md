...
The social preview image (`og-image.png`, 1200x630) is generated by a pure-stdlib Python script:

```sh
python3 scripts/gen_og_image.py
```

This writes `og-image.png` in the repo root. The script uses only `struct` + `zlib` to write a raw PNG, so no pip install is needed. `scripts/` is gitignored (build tools only), so just run it when you change the card design inside the script.
